...
public class Utility {
	...
	static OptionalInt eval(Expr expr) {
		...
		case App(var op, var l, var r) -> {
			var x = eval(l);
			var y = eval(r);
			yield (x.isPresent() && y.isPresent() && isValid(op, x.getAsInt(), y.getAsInt()))
	...
	static List<Result> combine(Result lx, Result ry) {
		...
		return Stream.of(operators).filter(op -> isValid(op, x, y))
				.map(op -> new Result(new App(op, l, r), apply(op, x, y))).toList();
	}
	...
	static List<List<Integer>> subs(List<Integer> ns) {
		...
		res.addAll(yss);
		yss.stream().map(l -> cons(x, l)).forEach(res::add);
	...
	static Stream<List<Integer>> choices(List<Integer> ns) {
		...
		return subs(ns).stream().flatMap(Utility::perms);
	}

	// solutions'' :: [Int] -> Int -> [Expr]
	// Using Stream<Expr> instead of List<Expr>
	public static Stream<Expr> solutions(List<Integer> ns, int n) {
		// solutions'' ns n = [e | ns' <- choices ns, (e,m) <- results' ns', m == n]
		return choices(ns).flatMap(choice -> results(choice).stream()).filter(res -> res.value() == n)
				.map(Result::expr);
	}
